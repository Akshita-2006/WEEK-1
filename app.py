...
st.markdown("""
    <div style="background-color: rgba(255,255,255,0.1); padding: 20px; border-radius: 15px;">
        <h1 style="text-align:center; color:brown;">🔋 EV Adoption Forecast</h1>
        <p style="text-align:center; font-size:18px; color:brown; font-weight:bold;">
            Forecast electric vehicle growth in Washington counties using smart machine learning predictions. 📈
        </p>
    </div>
""", unsafe_allow_html=True)
...
# Instruction line
st.markdown("""
    <div style='text-align: left; font-size: 22px; padding-top: 10px; color: brown; font-weight:bold;'>
...
if county not in df['County'].unique():
    st.markdown(
        """
        <div style="
            background-color: #ffe8e8;  /* soft pink */
            color: #8b0000;
        ...
        ">
            ...
            County '{county}' not found in dataset.
        </div>
        """, unsafe_allow_html=True
    )
    st.stop()
...
if historical_total > 0:
    ...
    <div style="
        ...
        margin-top: 1rem;
    ">
        Based on the graph, EV adoption in <strong>{county}</strong> is expected to show a 
        <strong>{trend} of {forecast_growth_pct:.2f}%</strong> over the next 3 years.
    </div>
    """, unsafe_allow_html=True)
    ...
    # st.success(f"Based on the graph, EV adoption in **{county}** is expected to show a **{trend} of {forecast_growth_pct:.2f}%** over the next 3 years.")
     ...
     st.markdown(
        ...
        ">
            <strong>Warning:</strong><br>
            Historical EV total is zero, so percentage forecast change can't be computed.
        </div>
        """, unsafe_allow_html=True
    )
...
with st.container():
    st.markdown("## 📊 Compare Up to 3 Counties")
    multi_counties = st.multiselect("Select Counties", county_list, max_selections=3)
...
st.markdown("""
---
<div style="text-align: center; color: brown; font-size: 14px;">
    🚀 Built for the AICTE Internship Cycle 2 by <b>S4F</b> | Made by Akshita Sachdeva 💡
</div>
""", unsafe_allow_html=True)